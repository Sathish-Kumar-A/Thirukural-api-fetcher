let firstline=document.querySelector(".kural1");
let secondline=document.querySelector(".kural2");
...
async function search(){
    try{
        ...
        console.log("fetch error in search")
    }
}
...
function details(result){
    ...
    //details
    container1.innerHTML=`<h5 class="tamil">அதிகாரம் : <small>${result.chap_tam}</small></h5>
...
async function previous(){
    try{
        ...
        if(x>0){
            ...
            console.log("Eetch Error In Previous")
        }
}
...
async function next(){
    try{
        ...
        console.log("Fetch Error in Next")
    }
}
